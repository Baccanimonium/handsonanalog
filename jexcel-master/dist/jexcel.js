...
var jexcel = function (el, options) {
  ...
  obj.createCell = function (i, j, value) {
    ...
    if (obj.options.columns[i].type == 'hidden') {
      ...
      td.innerHTML = jSuites.calendar.getDateString(formatted || value, obj.options.columns[i].options.format)
    ...
    if (cell.classList.contains('readonly') == true) {
      ...
      if (obj.options.columns[x].editor) {
        ...
        if (obj.options.columns[x].type == 'hidden') {
          ...
          var options = {
            data: source,
            multiple: !!obj.options.columns[x].multiple,
            autocomplete: !!(obj.options.columns[x].autocomplete || obj.options.columns[x].type == 'autocomplete'),
            opened: true,
            value: obj.options.columns[x].multiple ? value.split(';') : value,
            width: '100%',
            height: editor.style.minHeight,
            position: !!((obj.options.tableOverflow == true || obj.options.fullscreen == true)),
            onclose: function () {
  ...
  obj.closeEditor = function (cell, save) {
    ...
    if (save == true) {
      ...
      // Update values
      var ignoreEvents = !!obj.ignoreEvents
      var ignoreHistory = !!obj.ignoreHistory
    ...
    if (obj.records[y][x].classList.contains('readonly') == true && !force) {
      ...
      if (obj.options.columns[x].editor) {
        ...
        if (obj.options.columns[x].type == 'checkbox' || obj.options.columns[x].type == 'radio') {
          ...
          // Update data and cell
          obj.records[y][x].children[0].checked = !!((value == 1 || value == true || value == 'true'))
          ...
          // Update data and cell
          obj.options.data[y][x] = value
          obj.records[y][x].innerHTML = jSuites.calendar.getDateString(formatted || value, obj.options.columns[x].options.format)
        } else if (obj.options.columns[x].type == 'color') {
          // Update color
          obj.options.data[y][x] = value
          // Render
  ...
  obj.setComments = function (cellId, comments, author) {
    ...
    // Set new values
    obj.records[cell[1]][cell[0]].setAttribute('title', comments || '')
    obj.records[cell[1]][cell[0]].setAttribute('data-author', author || '')
